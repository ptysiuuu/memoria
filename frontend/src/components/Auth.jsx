...
const Auth = () => {
  ...
  const overlayBgLight = "bg-gradient-to-r from-pink-400 via-fuchsia-400 to-purple-400";
  const overlayBgDark = "dark:bg-gradient-to-r dark:from-pink-500 dark:via-fuchsia-600 dark:to-purple-700";

  return (
    <div className="h-4/5 w-4/5 relative bg-white dark:bg-gray-900 overflow-hidden rounded-lg font-primary">
      <div
        id="signin"
        className={`bg-white dark:bg-gray-800 absolute top-0 left-0 h-full w-1/2 flex justify-center items-center transition-all duration-700 ease-in-out z-20 ${isAnimated ? "translate-x-full opacity-0" : ""
          }`}
      >
        <SigninForm />
      </div>
      ...
      >
        <div
          id="overlay"
          className={`${overlayBgLight} ${overlayBgDark} relative -left-full h-full w-[200%] transform transition-transform duration-700 ease-in-out ${isAnimated ? "translate-x-1/2" : "translate-x-0"
            }`}
        >
          <div
            id="overlay-left"
